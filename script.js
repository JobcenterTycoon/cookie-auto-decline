...
(function () {
   if ((sessionStorage.getItem("mpowlesu908hxfyw37ghg5ikx90jdzt") != "djx0v0odce35xrb2pt5dzbgaj1mud5c" && (window.innerHeight > 400 || window.innerHeight === 0)) && document.querySelector("body.no-js > .main-wrapper + script + img[src^=\"/cdn-cgi/images/trace/managed/js/transparent.gif?ray=\"][style=\"display: none\"]") === null && document.querySelector("html[style=\"height:100%\"] iframe[src^=\"/_Incapsula_Resource?\"]") === null && document.querySelector("link[href=\"/cdn-cgi/styles/challenges.css\"][rel=\"stylesheet\"]") === null && window.location.hostname != "accounts.google.com") {
      ...
      window.setTimeout(function () {
         ...
         let csscheck2 = document.getElementById("6tr2bqvixssc0n08khsf3nfvf9urdax");
         if (csscheck1 != null) {
            csscheck1.remove();
         }
         if (csscheck2 != null) {
      ...
      const cookiedatum = datum.getFullYear() + "-" + datum.getMonth() + "-" + datum.getDay() + "T0:00:00.451Z";
      ...
      // cdn.privacy-mgmt.com (iframe klicker)
      if (window.self !== window.top) {
         if (window.location.host === "cdn.privacy-mgmt.com" || ((window.location.host.startsWith("consent") || window.location.host.startsWith("cmp") || window.location.host.startsWith("sp-cdn.") || window.location.host.startsWith("privacy.") || window.location.host.startsWith("baseendpoint.") || window.location.host.startsWith("spcmp.") || window.location.host.startsWith("sourcepoint") || window.location.host.startsWith("sp-consent.") || window.location.host.startsWith("sp.") || window.location.host.startsWith("cnsnt.")) && window.location.href.includes("/index.html?")) || window.location.host === "sp-spiegel-de.spiegel.de") {
            console.log("[Cookie auto decline] Detected: privacy-mgmt.com");
            const privacymgmt = window.setInterval(function () {
               const ablehnen = document.querySelector(".message-container button.sp_choice_type_REJECT_ALL[title][aria-label]");
               const ablehnen2 = document.querySelector(".message-container button[title*=\"blehnen\"][aria-label]");
      ...
      const findconsent = window.setInterval(function () {
         // consentmanager.net
         const consentmanager = document.querySelector("script[src=\"https://cdn.consentmanager.net/delivery/js/cmp_de.min.js\"][data-cmp-ab]");
         const consentmanager2 = document.querySelector("script[src^=\"https://cdn.consentmanager.net/delivery/customdata/\"][data-cmp-ab]");
         if (consentmanager || consentmanager2) {
            console.log("[Cookie auto decline] Detected: consentmanager.net");
            const ablehnen = document.querySelector("#cmpbox a.cmptxt_btn_no[role=\"button\"]");
            const akzeptieren = document.querySelector("#cmpbox a.cmptxt_btn_yes[role=\"button\"]");
            const einstellungen = document.querySelector("#cmpbox:not(:has(.cmpcontentpasssignuplogo[data-cmp-ab])) a.cmptxt_btn_settings[role=\"button\"]");
            const speichern = document.querySelector("#cmpbox a.cmptxt_btn_save[role=\"button\"]");
            const shadowroot = document.querySelector("#cmpwrapper");
            if (ablehnen) {
               beenden();
               window.setTimeout(function () {
                  ablehnen.click();
               ...
               window.setTimeout(function () {
                  const akzeptierenshadowroot = shadowroot.shadowRoot.querySelector("#cmpbox a.cmptxt_btn_yes[role=\"button\"]");
                  const ablehnenshadowroot = shadowroot.shadowRoot.querySelector("#cmpbox a.cmptxt_btn_no[role=\"button\"]");
                  if (ablehnenshadowroot && ablehnenshadowroot.offsetHeight > 0) {
                     beenden();
                     ablehnenshadowroot.click();
                  } else if (akzeptierenshadowroot && akzeptierenshadowroot.offsetHeight > 0) {
                     beenden();
                     akzeptierenshadowroot.click();
                  }
               }, 502);
            }
         }
         ...
         if (consensu) {
            ...
            const speichern = document.querySelector("#cmpbox a.cmptxt_btn_save[role=\"button\"]");
            const speichern2 = document.querySelector("#cmpbox a.cmpboxbtnyes");

            const shadow = document.getElementById("cmpwrapper");
            let ablehnenshadow;
            let einstellungenshadow;
            let speichernshadow;
            if (shadow) {
               ablehnenshadow = document.getElementById("cmpwrapper").shadowRoot.querySelector("#cmpbox a.cmptxt_btn_no[role=\"button\"]");
               einstellungenshadow = document.getElementById("cmpwrapper").shadowRoot.querySelector("#cmpbox a.cmptxt_btn_settings[role=\"button\"]");
               speichernshadow = document.getElementById("cmpwrapper").shadowRoot.querySelector("#cmpbox a.cmptxt_btn_save[role=\"button\"]");
            }
            if (ablehnen) {
               beenden();
               window.setTimeout(function () {
            ...
            } else if (ablehnenshadow) {
               beenden();
               window.setTimeout(function () {
                  ablehnenshadow.click();
               }, 502);
            } else if (einstellungen) {
               window.setTimeout(function () {
                  einstellungen.click();
               }, 502);
            } else if (einstellungenshadow) {
               window.setTimeout(function () {
                  einstellungenshadow.click();
               }, 502);
            } else if (speichern2) {
               beenden();
               window.setTimeout(function () {
            ...
            } else if (speichernshadow) {
               beenden();
               window.setTimeout(function () {
                  speichernshadow.click();
               }, 502);
            }
         }
         ...
         // borlabs.io
         const borlabs = document.querySelector("script#BorlabsCookieBoxWrap");
         const borlabs2 = document.querySelector("script#borlabs-cookie-js");
         const borlabsverify = document.querySelector("#BorlabsCookieBox > .bottom-center[style=\"display: none;\"]");
         if ((borlabs || borlabs2) && borlabsverify === null) {
            console.log("[Cookie auto decline] Detected: borlabs.io");
            const speichern = document.querySelector("#BorlabsCookieBox ._brlbs-accept > a#CookieBoxSaveButton[data-cookie-accept]");
            if (speichern) {
               beenden();
               window.setTimeout(function () {
                  speichern.click();
         ...
         if (privacycenter || privacycenter2) {
            ...
            const akzeptieren = document.querySelector("#didomi-host #didomi-popup #buttons > button#didomi-notice-agree-button");
            if (ablehnen) {
               beenden();
               window.setTimeout(function () {
                  ablehnen.click();
            ...
            } else if (einstellungen2 && einstellungen2.offsetHeight > 0) {
               window.setTimeout(function () {
                  einstellungen2.click();
               }, 402);
            } else if (akzeptieren) {
         ...
         if (iubenda && document.cookie.includes("_iub_cs-") === false) {
         ...
         if (cookieconsentdesc && window.location.host.endsWith("sozialversicherung-kompetent.de") === false) {
            ...
            } else if (ablehnen3 && ablehnen3.offsetHeight > 0) {
               beenden();
               window.setTimeout(function () {
                  ablehnen3.click();
               }, 202);
         ...
         if (ccm19de && localStorage.getItem("ccm_consent") === null) {
            ...
            window.setTimeout(function () {
               if (localStorage.getItem("ccm_consent") === null) {
                  localStorage.setItem("ccm_consent", "{\"/\":{\"gen\":2,\"ucid\":\"20573d3e28bd2b2d\",\"consent\":true,\"embeddings\":[\"e553470\",\"ff1dff6\",\"7ccf7a0\"],\"created\":" + Date.now() + ",\"updated\":" + Date.now() + ",\"iframeConsentDomains\":[],\"tcf\":{\"p\":[],\"sf\":[],\"v\":[],\"gad\":[]},\"lang\":\"de-DE\"}}");
                  beenden();
                  location.reload();
               }
            }, 502);
         ...
         // ckyBannerTemplate
         const ckyBannerTemplate = document.querySelector("script#ckyBannerTemplate");
         if (ckyBannerTemplate) {
            console.log("[Cookie auto decline] Detected: ckyBannerTemplate");
            const ablehnen = document.querySelector(".cky-consent-bar button.cky-btn-reject");
            if (ablehnen) {
               beenden();
               window.setTimeout(function () {
                  ablehnen.click();
         ...
         if (acris && document.cookie.includes("cookie-preference") === false) {
         ...
         // klaro.org 2
         const klarozwei = document.querySelector("div[id=\"timm4-cookie-consent\"] > div[class^=\"klaro\"]");
         if (klarozwei && document.cookie.includes("timm4cookie-consent") === false) {
         ...
         const consentframework = document.querySelector("#sd-cmp[class^=\"sd-cmp-\"]");
         if (consentframework && document.cookie.includes("euconsent-v2") === false) {
            const ablehnen = consentframework.querySelector("button:only-child");
            if (ablehnen) {
               beenden();
               window.setTimeout(function () {
                  ablehnen.click();
      ...
      const regeln = [{
         ...
         seite: "europa.eu",
         selector: ".cck-actions-button[href=\"#refuse\"]",
         selectorfinal: "div.cck-actions > button.wt-ecl-message__close"
      }, {
         seite: "autismus.de",
         ...
         setcookie: "cookie_settings={\"necessary\":true,\"statistics\":false,\"marketing\":false};"
      }, {
         ...
         seite: "kurier.at",
         selector: "app-required-consent-overlay button.hasBackground"
      }, {
         ...
         seite: "tum.de",
         selector: "button[data-in2-modal-save-button]"
      }, {
         ...
         seite: "facebook.com,meta.com",
         selector: "button[data-cookiebanner=\"accept_only_essential_button\"]",
         selectorfinal: "div[role=\"dialog\"] div[aria-label$=\"blehnen\"][role=\"button\"]"
      }, {
         seite: "twitter.com",
         selector: "div[class*=\"eu-cookie-notice\"] ul[class$=\"-eu-cookie-notice__choice-list\"] button[class*=\"-eu-cookie-notice__button\"]",
         selectoralternate: "div[role=\"button\"][tabindex][style] + div[role=\"button\"][tabindex][style]",
         selectorfinal: "div[data-cname*=\"_opt\"] ul[class$=\"-opt-in__choice-list\"] button.is-naked",
         noframe: true
      }, {
         seite: "instagram.com",
         ...
         seite: "whatsapp.com",
         selector: "div[data-testid=\"wa_cookies_banner_modal\"] button[data-testid=\"cookie_banner_accept_button\"]"
      }, {
         seite: "linkedin.com",
         ...
         seite: "bing.com",
         selector: "#bnp_cookie_banner button#bnp_btn_reject"
      }, {
         seite: "etsy.com",
         checkcookie: " p=",
         selector: "#gdpr-single-choice-overlay button[data-gdpr-open-full-settings]",
         selectorfinal: "#gdpr-privacy-settings button[data-wt-overlay-close]"
      }, {
         seite: "rp-online.de",
         ...
         seite: "edna.de",
         checkstorage: "econda.privacy.protection",
         selector: "#privacyProtectionBanner button#buttonOK"
      }, {
         seite: "gamestar.de",
         ...
         seite: "forbes.com",
         selector: "div[role=\"dialog\"][aria-modal][aria-describedby][aria-labelledby][style] > div[class^=\"buttons__\"] > button[class*=\"secondaryButton\"]"
      }, {
         seite: "poco.de",
         selector: "#jentis_consent center > a[variant=\"link\"]"
      }, {
         seite: "wayfair.de",
         selector: ".CookieComplianceContainer-veil button[type=\"button\"][data-enzyme-id=\"BannerCookieSettings\"]",
         selectorfinal: "div[data-enzyme-id=\"CookieComplianceModal\"] button[data-enzyme-id=\"ModalSaveAndClose\"]"
      }, {
         seite: "otto.de",
         checkcookie: "cb=",
         ...
         selectorfinal: "span.a-declarative[data-sp-cc][data-action=\"sp-cc\"] > form#sp-cc[action^=\"/privacyprefs/\"] > #sp-cc-rejectall-container > a#sp-cc-rejectall-link[href=\"#\"][role=\"button\"]",
         countdown: 1000
      }, {
         seite: "aliexpress.com",
         checkcookie: "u_p_s=2",
         selector: "#gdpr-new-container.global-gdpr-container button.btn-more",
         selectorfinal: ".gdpr-dialog-wrap .gdpr-dialog-btn-group > button.btn-save"
      }, {
         seite: "dan.com",
         ...
         seite: "dw.com",
         checkstorage: "gdprAccepted",
         selector: ".gdpr-wrapper .button-wrapper div.primary[role=\"button\"]"
      }, {
         seite: "fainin.com",
         checkcookie: "ucc_consents",
         selector: "fainin-cookie-consent button.cookie_consent-button--secondary"
      }, {
         seite: "0rechner.de,0calc.com,0calc.es,0calc.fr",
         checkcookie: "precmp",
         selector: "#precmpdialog form[action=\"/precmp\"] button[value=\"deny\"]"
      }, {
         seite: "stackoverflow.com,askubuntu.com,mathoverflow.net,serverfault.com,stackexchange.com,superuser.com",
         checkcookie: "Optanon",
         selector: "body > .js-consent-banner button.js-reject-cookies"
      }, {
         seite: "norton.com",
         ...
         seite: "blick-store.de",
         checkstorage: "consent",
         selector: "#consent-manager button#consent-banner-btn-close.consent-btn-outline-primary[type=\"button\"]"
      }, {
         seite: "keyed.de",
         ...
         selector: "#cookie_accept_msg_block button[onclick=\"cookieMsg.set(\'accepted\')\"]"
      }, {
         seite: "lionic.com",
         ...
         selector: ".mnd-cookie-modal button.btn-accept-functional"
      }, {
         ...
         seite: "supermarkt24h.de",
         checkstorage: "consent",
         selector: "#consent-manager button#consent-banner-btn-close[type=\"button\"]"
      }, {
         ...
         checkcookie: "necessary-cookie-",
         selector: "form#form-cookies-banner button.btn-accept-necessary"
      }, {
         ...
         seite: "metro.hu",
         checkcookie: "cookieconsent",
         selector: ".cookie-disclaimer-intrusive button.btn-primary.accept-btn"
      }, {
         seite: "filmzie.com",
         ...
         seite: "abo24.de",
         checkcookie: "cmp",
         selector: "#cookie-consent-wrapper a#consent-all[class=\"button\"][onclick=\"cookieConsentAll();\"]"
      }, {
         seite: "lolaliza.com",
         checkcookie: "enableTracking",
         selector: "#cookie-notice a#cookie-bar-button.cookieBar_close"
      }, {
         seite: "werbringtwas.com",
         checkcookie: "cookieconsent_wbw_status",
         selector: "#cookie-notice a#cookie-bar-button.cookieBar_close"
      }, {
         ...
         selector: ".cookie-panel__description button.cookie-panel__button.button_min"
      }, {
         ...
         seite: "umicore.jp",
         checkcookie: "CookieConsent",
         selector: "#cookieconsent-banner a#cookieconsent-banner-accept-necessary-button"
      }, {
         seite: "coqtales.com",
         ...
         seite: "deutsche-islam-konferenz.de,digital-fuer-deutschland.de,bvl.bund.de,auslandsschulwesen.de,bamf.de,thw.de,itzbund.de,bafin.de,personalausweisportal.de",
         ...
         seite: "esvika.ee",
         checkcookie: "Consent",
         selector: "#cookieConsent button[type=\"button\"][data-cookie-string]"
      }, {
         seite: "schell.eu",
         ...
         selectorfinal: ".modal-dialog button.btn-secondary",
         countdown: 1502
      }, {
         ...
         selector: ".cookies-wrapper a.cookies[data-cookies-action=\"accept\"]"
      }, {
         seite: "soluxtec.de",
         ...
         seite: "bitburger.de",
         checkcookie: "cookies-general",
         selector: ".cookie-age button#cookie-layer-continue"
      }, {
         seite: "bsh-group.com",
         ...
         selector: "#tarteaucitronServices button#tarteaucitronSave[type=\"button\"]"
      }, {
         ...
         seite: "myfxbook.com",
         checkcookie: "cookieconsent_status",
         selector: "#gdprConsentBanner button#dismissGdprConsentBannerBtn"
      }, {
         seite: "cgi.com",
         ...
         selector: "iframe + div div[style^=\"max-height:\"] > div:last-child button[type=\"button\"]",
         selectorfinal: "iframe + div button:not([aria-label]):first-child"
      }, {
         seite: "frankfurt.de",
         ...
         seite: "bitburger-braugruppe.de",
         checkcookie: "cookie-",
         selector: "#cookie-layer-wrapper a#cookie-accept"
      }, {
         ...
         checkcookie: "Marketing_consent=0",
         setcookie: "Marketing_consent=2;",
         selector: "wc-cookies-marketing #cookie-m-button-allow"
      }, {
         seite: "simyo.nl",
         ...
         selector: ".cookie-information button[type=\"button\"][data-xo-cookieinformation-href*=\"cookies_type=technical\"]"
      }, {
         ...
         selector: ".js-cookie-notification-settings-container button[aria-label*=\"kzeptieren\"]"
      }, {
         seite: "gronkh.tv",
         ...
         selector: ".cc-dialog button.cc-dialog-button-decline"
      }, {
         seite: "sehen.de",
         ...
         seite: "weimarerland.de,weimar.de",
         selector: "#gccookiemessage button#cookieSubmitButton[onclick][type=\"button\"]"
      }, {
         seite: "smart.com",
         checkcookie: "OptanonAlertBoxClosed",
         selector: "#cookie-pro-dialog button#cookie-pro-reject-button"
      }, {
         seite: "websummit.com",
         selector: ".civic_cookie__banner .civic_cookie__button--rejectAll"
      }, {
         ...
         seite: "web.de,gmx.net",
         checkcookie: "consentLevel",
         sonderfall: "demailsites",
         noframe: true
      }, {
         seite: "zeit.de",
         checkcookie: "zonconsent",
         sonderfall: "zeitde"
      }, {
         seite: "tiktok.com",
         ...
         seite: "kadari.net",
         checkcookie: "cookiebar",
         selector: ".cookie-warning a.cookie-warning-close"
      }, {
         ...
         selector: "#cookie-consent-modal button.btn-secondary[type=\"button\"]",
         selectorfinal: "#cookie-consent-modal button.btn-primary[type=\"button\"]"
      }, {
         seite: "swindi.de",
         ...
         seite: "fitx.de",
         checkcookie: "consent",
         selector: ".cookie_overlay div.cookie_overlay__button--mandatory"
      }, {
         seite: "erdinger.de",
         ...
         checkcookie: "essential",
         selector: "#gdpr-dialog a.gdpr-extend[href=\"#\"]",
         ...
         selector: "#GDPRConsentManagerContainer #ccmgt_explicit_preferences.options-button",
         ...
         selectorfinal: "#cboxContent a.js_cm_consent_essentials_only",
         countdown: 1000
      }, {
         ...
         seite: "huf-haus.com",
         checkstorage: "ccm_consent",
         selector: "#ccm-widget button.ccm--decline-cookies"
      }, {
         seite: "ovpn.com",
         checkcookie: "eovpn_cookie_consent",
         selector: ".cookie-consent button.js-cookie-consent-agree"
      }, {
         ...
         seite: "azirevpn.com",
         checkcookie: "az_hide_cookie_bar",
         selector: "#cookiebar button"
      }, {
         ...
         seite: "hidemy.io",
         checkstorage: "gdprconfirm",
         selector: "#gdpr-block button#gdpr-confirm-btn"
      }, {
         seite: "wp.pl,money.pl,o2.pl,parenting.pl,pudelek.pl,autokult.pl,gadzetomania.pl,fotoblogia.pl,komorkomania.pl,polygamia.pl,abczdrowie.pl,benchmark.pl,kafeteria.pl,pysznosci.pl,dobreprogramy.pl,genialne.pl,autocentrum.pl",
         ...
         seite: "telekom.com",
         checkcookie: "CONSENTMGR",
         selector: "#__tealiumGDPRecModal button#rejectAll"
      }, {
         seite: "phase-6.de",
         checkcookie: "GDPR",
         ...
         seite: "reifen-4x4.de",
         checkcookie: "PrestaShop-4972",
         selector: "#acb-banner button#acb-close-button"
      }, {
         ...
         seite: "christy.co.uk",
         checkcookie: "cookiesNotification",
         selector: "#cookies-wrapper #cookies-dismiss > span"
      }, {
         ...
         checkcookie: "cookieconsent",
         selector: "#iw-cookie-banner #customize-cookies-btn",
         selectorfinal: "#customize-cookie-settings .customize-cookies-close:last-of-type"
      }, {
         seite: "destatis.de",
         checkcookie: "gsbbanner",
         ...
         seite: "tinyurl.com",
         checkstorage: "cookiesAccepted",
         selector: "div[class=\"alert alert-info alert-dismissible text-center m-0\"] > button.btn-sm"
      }, {
         seite: "webnode.com",
         checkcookie: "essential",
         ...
         checkcookie: "_evidon_suppress_notification_cookie",
         selector: "#_evidon_banner button#_evidon-decline-button"
      }, {
         seite: "playstation.com",
         checkcookie: "_evidon_suppress_notification_cookie",
         setcookie: "_evidon_suppress_notification_cookie={\"date\":\"2023-09-14T19:12:41.067Z\"};",
         selector: "#_evidon_banner button#_evidon-decline-button"
      }, {
         ...
         selector: ".glue-cookie-notification-bar button.glue-cookie-notification-bar__reject"
      }, {
         seite: "rt.com",
         ...
         selector: ".b-policy-info__wrapper .b-policy-info__cross"
      }, {
         seite: "ionos.de",
         ...
         selector: ".wz-cc-container button.wz-cc-disallow"
      }, {
         seite: "123rf.com",
         ...
         seite: "safety.google",
         checkstorage: "glue.CookieNotificationBar",
         selector: "div[id^=\"glue-cookie-notification-bar-\"] button.glue-cookie-notification-bar__reject"
      }, {
         seite: "xing.com",
         ...
         seite: "neuwied.de,gotha.de",
         checkcookie: "cookieconsent_dismissed",
         selector: ".cc_banner > .cc_btn_accept_all[data-cc-event]"
      }, {
         seite: "radebeul.de",
         ...
         seite: "a-coding-project.de",
         checkcookie: "cookieconsent_status",
         selector: "#cookiebox #change-cookies"
      }, {
         seite: "get-in-it.de",
         ...
         seite: "lufthansa.com",
         checkcookie: "CONSENTMGR",
         selector: ".consent-manager #cm-acceptNone"
      }, {
         seite: "futurezone.at",
         checkcookie: "__pid",
         ...
         checkstorage: "aa_consentGroup",
         selectorfinal: ".aa-first-layer__content button.aa-first-layer__button[onclick=\"acceptPUR()\"]",
         countdown: 1500
      }, {
         seite: "news.samsung.com",
         checkcookie: "COOKIECONSENT",
         selector: "#bgc-cookie-popup #cookie_action_reject"
      }, {
         seite: "moemax.de",
         ...
         seite: "databroker.global",
         checkcookie: "databroker_cookie",
         selector: ".cookie-accept button#cookie-accept-btn"
      }, {
         seite: "movistar.es",
         checkcookie: "OptanonAlertBoxClosed",
         ...
         for (let k = 0; k < seiten.length; k++) {
            if (location.hostname.endsWith("." + seiten[k]) || location.hostname === seiten[k]) {
               ...
               if (document.cookie.includes(regeln[i].checkcookie) === false && localStorage.getItem(regeln[i].checkstorage) === null && ((regeln[i].noframe === true && window.self == window.top) || regeln[i].noframe != true)) {
                  ...
                  // Sonderfälle
                  if (regeln[i].sonderfall === "zeitde") {
                     if (window.location.href.includes("www.zeit.de/zustimmung?url=") === true) {
                        document.cookie = "zonconsent = " + cookiedatum + " ; domain=.zeit.de; secure=true; max-age=31104000; SameSite=Lax; path=/";
                        let url = decodeURIComponent(window.location.href);
                        let formatierteurl = url.replace("https://www.zeit.de/zustimmung?url=", "");
                        if (formatierteurl.startsWith("https://www.zeit.de/")) {
                           window.location.replace(formatierteurl);
                        } else {
                           window.location.replace("https://www.zeit.de/");
                        }
                     }
                  } else if (regeln[i].sonderfall === "tiktokcom") {
                     window.setTimeout(function () {
                     ...
                     window.setTimeout(function () {
                        ...
                        if (check) {
                           const akzeptieren = check.contentWindow.document.querySelector("html button.button__acceptAll");
                           if (akzeptieren) {
                              akzeptieren.click();
                           }
                        }
                     ...
                     document.cookie = "euconsent-v2=CP5P4cAP5P4cACnABIDEAlEkAP_.f_wAAAAABcgAAAAA;" + nc;
                     if (document.location.href.endsWith("consent-management/")) {
                        forcesessionstorage();
                        document.cookie = "euconsent-bypass=1;" + nc;
                        document.cookie = "consentLevel=3;" + nc;
                        let createinlinescript = document.createElement("script");
                        createinlinescript.innerText = "if (window.ui && window.ui.redirectUrl) { location.replace(window.ui.redirectUrl); } else { location.replace(location.origin); }";
                        document.body.append(createinlinescript);
                     } else {
                        window.setTimeout(function () {
                           document.cookie = "euconsent-bypass=1;" + nc;
                           document.cookie = "consentLevel=3;" + nc;
      ...
      if (window.location.host.endsWith(".de") && (window.location.host.includes("sparkasse-") || window.location.host.includes("stadtsparkasse-") || window.location.host.startsWith("www.ssk") || window.location.host.startsWith("www.spk") || window.location.host.endsWith("haspa.de")) && document.cookie.includes("ePrivacyStatus") === false) {
         window.setTimeout(function () {
            const check = document.querySelector("div[class*=\"privacy\"] [style] .ebutton > a.primary[data-form=\".eprivacy_optin_decline\"]");
            if (check) {
               check.click();
            }
         }, 202);
