...
(function () {
   ...
   const findecookiebannerhauptscript = function () {
      ...
      let paybuttons = function (c) {
         ...
         const paytext = ['kaufe', 'bestell', 'kostenpflichtig', 'einloggen', 'abo', 'abbonati', 'pur ', 'pur-', 'werbefrei', 'spende', 'buchen', 'buy', 'pay', '€', '$', 'adfree', 'ad free', 'ad-free', 'subscribe', 'pro ', 'premium', 'order ', 'login', 'einloggen', 'anmelden', 'regist', 'donate', 'suscrib', 'pagar', 'download', 'contentpass', 'content-pass', 'content pass', 'mitgliedschaft', 'profil'];
         for (let i = 0; i < paytext.length; i++) {
            if (c.includes(paytext[i]) || c.match(/[0-9]/) || c === 'order' || c.endsWith(' pur')) {
               return true;
            }
         }
      };
         ...
         findconsent = function () {
            ...
            if (window.self !== window.top) {
               ...
               if (window.location.href.includes('/index.html?') && window.location.href.includes('hasCsp=') && window.location.href.includes('message_id=') && window.location.href.includes('consent')) {
                  ...
                  if (!akzeptieren) {
                     akzeptieren = document.querySelector('button.accept-all, button.sp_choice_type_ACCEPT_ALL, .sp_choice_type_11');
                  }
                  if (aboerforderlich) {
                     ablehnen = undefined;
            ...
            if (cmpbox) {
               ...
               if (cookieeinstellung === 'funktional') {
                  switchesdelay++;
                  const boxen = cmpbox.querySelectorAll(':is(tr, .cmpwelcomeprps):has(a[role="checkbox"])');
                  for (let i = 0; i < boxen.length; i++) {
                     const b = boxen[i].querySelector('span[id^="cmpboxvendname"], td.cmpvendname, .cmpwelcomeprpstxt');
                     if (knöpfetextcheck(b) === 'funktionaltext') {
            ...
            // trustarc.com
            const trustarc = document.querySelector('.truste_box_overlay, #truste-consent-track');
            if (trustarc && document.cookie.includes('notice_gdpr_prefs=') === false && document.querySelector('script[src^="https://consent.trustarc.com/asset/notice.js/v/v"]')) {
               console.log('[Cookie auto decline] Detected: trustarc.com');
               cookiebannerstatus.anbieter = 'trustarc.com';
               nureinklickeinstellungen = true;
               advancedrun = false;
               if (bereitsgeklickt === false) {
                  ablehnen = trustarc.querySelector('button#truste-consent-required, a#truste-consent-required');
                  akzeptieren = trustarc.querySelector('button#truste-consent-button, a#truste-consent-button');
               }
               einstellungen = trustarc.querySelector('button#truste-show-consent, a#truste-show-consent');
               let shadowroot = trustarc.querySelector('.truste_popframe');
               if (shadowroot && shadowroot.shadowRoot) {
                  shadowroot = shadowroot.shadowRoot;
                  if (bereitsgeklickt === false) {
                     ablehnen = shadowroot.querySelector('.declineAllButtonLower, a.required[role="button"], a.rejectAll');
                     akzeptieren = shadowroot.querySelector('.acceptAllButtonLower, a.call[role="button"], a.acceptAll');
                     einstellungen = shadowroot.querySelector('a.shp');
                     window.setTimeout(function () {
                        const bestätigungsfenster = shadowroot.querySelector('a#gwt-debug-close_id[role="button"], button#gwt-debug-close_id');
                        if (bestätigungsfenster && bestätigungsfenster.checkVisibility) {
                           bestätigungsfenster.click();
                        }
                     }, 1002);
                  }
               }
               if (cookieeinstellung === 'funktional') {
                  let einstellungshadowroot = document.querySelector('.truste_box_overlay .truste_popframe');
                  if (einstellungshadowroot && einstellungshadowroot.shadowRoot) {
                     einstellungshadowroot = einstellungshadowroot.shadowRoot;
                     const alleswitchesablehnen = einstellungshadowroot.querySelectorAll('[aria-disabled="false"]:is(.consentno, [class^="gwt-"].on):not([aria-pressed="true"], [aria-checked="true"], [disabled], .active, .activeConsent, [rtayztwplpnfftousydd])');
                     for (let i = 0; i < alleswitchesablehnen.length; i++) {
                        if (alleswitchesablehnen[i].checkVisibility()) {
                           alleswitchesablehnen[i].setAttribute('rtayztwplpnfftousydd', 'a');
                           alleswitchesablehnen[i].click();
                        }
                     }
                     const boxen = einstellungshadowroot.querySelectorAll('li.listItem, .categoryPanelAccessible, .cookiecat');
                     if (boxen.length >= 1) {
                        switchesdelay += 0.5;
                     }
                     for (let i = 0; i < boxen.length; i++) {
                        const b = boxen[i].querySelector(':is([translate][class="gwt-InlineHTML"], .purposesListText, h2, h3, .viewcookie):not([style="visibility: visible;"], [aria-pressed="true"], [aria-checked="true"], [disabled], .active, .activeConsent, .on, .off)');
                        if (knöpfetextcheck(b) === 'funktionaltext') {
                           const checkbox = boxen[i].querySelector('[aria-disabled="false"]:is(.consentyes, [class^="gwt-"].off):not([aria-pressed="true"], [aria-checked="true"], [disabled], .active, .activeConsent)');
                           if (checkbox) {
                     ...
                     speichern = einstellungshadowroot.querySelector('a.saveAndExit, button.submitPreferencesButtonLower, a.submit[role="button"], button.submit');
                     if (switchesdelay >= 4) {
                        window.setTimeout(function () {
                           const bestätigungsfenster = einstellungshadowroot.querySelector('a#gwt-debug-close_id[role="button"], button#gwt-debug-close_id');
                           if (bestätigungsfenster && bestätigungsfenster.checkVisibility) {
                              bestätigungsfenster.click();
                           }
                        }, 1002);
            ...
            // iubenda.com
            const iubenda = document.querySelector('#iubenda-cs-banner, tb-banner-wrapper');
            if (iubenda && document.cookie.includes('%7B%22timestamp%22%') === false) {
               ...
               if (iubenda.shadowRoot) {
                  let iubendashadowroot = iubenda.shadowRoot.querySelector('tb-banner-footer');
                  if (iubendashadowroot && iubendashadowroot.shadowRoot) {
                     iubendashadowroot = iubendashadowroot.shadowRoot.querySelectorAll('tb-action-button');
                     for (let i = 0; i < iubendashadowroot.length; i++) {
                        if (iubendashadowroot[i].shadowRoot) {
                           if (iubendashadowroot[i].shadowRoot.querySelector('button.reject-button')) {
                              ablehnen = iubendashadowroot[i].shadowRoot.querySelector('button.reject-button');
                           } else if (iubendashadowroot[i].shadowRoot.querySelector('button.accept-button')) {
                              akzeptieren = iubendashadowroot[i].shadowRoot.querySelector('button.accept-button');
                           }
                        }
                     }
                  }
                  let iubendashadowrooteinstellungen = iubenda.shadowRoot.querySelector('tb-tabs-container');
                  if (iubendashadowrooteinstellungen && iubendashadowrooteinstellungen.shadowRoot) {
                     iubendashadowrooteinstellungen = iubendashadowrooteinstellungen.shadowRoot.querySelectorAll('tb-banner-tab:not([active], [button-id="cmp-cs-title"])');
                     for (let i = 0; i < iubendashadowrooteinstellungen.length; i++) {
                        if (iubendashadowrooteinstellungen[i].shadowRoot && iubendashadowrooteinstellungen[i].shadowRoot.querySelector('button:not([disabled])') && iubendashadowrooteinstellungen[i].shadowRoot.querySelector('button:not([disabled])').checkVisibility()) {
                           einstellungen = iubendashadowrooteinstellungen[i].shadowRoot.querySelector('button:not([disabled])');
                        }
                     }
                  }
               }
            ...
            if (jtlsoftwarede && (window.localStorage.getItem('consent') === null || cookieeinstellung === 'funktional')) {
               ...
               speichern = document.querySelector('#consent-settings :is(button#consent-accept-banner-btn-close, button.consent--modal-close, button[data-toggle="consent-close"].consent-btn-primary:not(.d-md-none))');
               if (cookieeinstellung !== 'funktional') {
                  ablehnen = jtlsoftwarede.querySelector('#consent-manager.active button#consent-banner-btn-close, #consent.active button#consent--banner-btn-decline');
                  akzeptieren = jtlsoftwarede.querySelector('#consent.active button#consent--banner-btn-accept, #consent-manager.active button#consent-banner-btn-all');
               }
               if (cookieeinstellung === 'funktional') {
                  switchesdelay++;
               ...
               for (let i = 0; i < advancedcontainer.length; i++) {
                  ...
                  if (sichtbarkeitsprüfung(a) === true && (b === 'fixed' || b === 'sticky' || bp === 'fixed' || bp === 'sticky' || bc === 'fixed' || bc === 'sticky' || window.getComputedStyle(document.querySelector('body')).getPropertyValue('overflow') === 'hidden' || a.outerHTML.toLowerCase().includes('cookie') === true) && a.childElementCount >= 1 && a.innerText.length > 10) {
                     ...
                     if (a.querySelector('a[href*="cookie" i]') !== null ||
                        ...
                        const knöpfe = a.querySelectorAll('button:not(:disabled), a:not([href*="/"]), a[href*="optout" i], a[class*="button"], [role="button"], input[type="button"], input[type="submit"], span[class*="button"], :is([class*="dismiss"], [id*="dismiss" i], [class*="preferences" i], [id*="preferences" i], [class*="submit"], [id*="submit"], [class*="close" i], [id*="close" i], [class*="reject" i], [id*="reject" i], [class*="decline" i], [id*="decline" i], [class*="mandatory"], [id*="mandatory"], [class*="accept" i], [id*="accept" i], [data-action*="accept" i], [id*="ausblenden" i], [class*="ausblenden" i], [id*="essential" i], [class*="essential" i], [id*="chose_no"], [class*="chose_no"], [title*="ablehnen" i]):is(span, div):not(:has(button, a, input[type="checkbox"]))');
                  ...
                  if (advancedslowdown === 0) {
                     ...
                     for (let i = 0; i < elemente.length; i++) {
                        ...
                        if (elementtext && elementtext.length >= 50 && elementtext.length < 3500 && (elementhtml.includes('cookie') || (document.documentElement.lang && webseitesprache === 'tr' && elementtext.includes('çerezler')))) {
                           ...
                           const inputs = elemente[i].querySelectorAll('input:not([type="button"], [type="checkbox"], [type="hidden"], [type="radio"], [type="submit"], [type="image"], [readonly]:not([type]))');
                           let inputs2 = [];
